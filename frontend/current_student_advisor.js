(() => {
  ...
  function selectProgramContext(){sessionStorage.setItem('selectedProgramContext',JSON.stringify({institutionCode:'BMCC',programCode:program.code,studentStatus:'current',onboardingSource:'narrative'}));}
  async function init(){
    if(!selected?.programCode){location.replace('/');return;}
    const response=await fetch('/api/db/programs?selector_only=true');const programs=await response.json();
    program=programs.find(item=>item.institution_code==='BMCC'&&item.code===selected.programCode);
    if(!program){location.replace('/');return;} selectProgramContext();
    ...
    renderTools(document.getElementById('all-tools'),Object.values(ACTIONS));
  ...
  document.body.addEventListener('click',event=>{const link=event.target.closest('[data-action="tree"]');if(!link)return;event.preventDefault();if(program&&window.CurriculumGraph)window.CurriculumGraph.open(program.code,{completedCourseCodes:[]});});
  ...
  form.addEventListener('submit',async event=>{
    ...
    try{const response=await fetch('/api/current-student-advisor/ask',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({user_question:text,program,conversation_summary:intake?JSON.stringify({current_major:intake.current_major,goal_type:intake.goal_type}):''})});const data=await response.json();if(!response.ok)throw new Error(data.detail||'The AI advisor is unavailable.');addMessage('assistant',data.answer||'No answer was returned.');renderTools(document.getElementById('recommended-tools'),data.recommended_actions||[]);}catch(error){addMessage('error',error.message);}finally{askButton.disabled=false;askButton.textContent='Ask';question.focus();}
  });
