...
- **History requirement (Program Requirements, 3 credits).** Both maps state:
  ...
  SOC." Modeled as placeholder `ECO-AA-HISTORY` using the program-specific
  derived group `ECO_AA_HISTORY`. The group derives from the canonical BMCC
  general-elective pool and restricts membership to the approved subject
  prefixes `HIS|ANT|GEO|PHI|POL|PSY|SOC`, so the selector is complete without
  changing the college-wide pool.
...
2. ECO 202's MAT-206-eligibility placement rule and the ENG 100.5
   reading-exemption placement rule are both advisory-only in the current
   data model (see "Prerequisite review").
3. The 4-credit General Elective / STEM-excess-credit mechanism is
