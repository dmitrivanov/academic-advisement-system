...
- **MAT 150 (Common Core, Mathematical and Quantitative Reasoning).** Both
  degree maps require MAT 150 (Introduction to Statistics) specifically —
  the five-semester map's equivalent is MAT 150.5 (Statistics with Algebra),
  footnoted as satisfying the same requirement. Modeled with
  `choice_group_code=CRJ_AA_MATH_QUANT`, a derived group in
  `program_choice_group_adjustments.csv` restricted to MAT 150 (4 credits).
  MAT 150.5 (five-semester-only alternate) was not added as an additional
  row, matching the established ENG 100.5/MAT 161.5 handling in Economics
  and History. **Flagged for maintainer review** if five-semester-track
  students need explicit support.
...
- **English Elective or LIN 250 (Forensic Linguistics).** Footnote: "Any
  300-level English course is an alternate option." LIN 250 is modeled as
  the literal required row; the "any 300-level English course" alternate is
  **not encoded** because no specific course list is published for it and
  enumerating "every 300-level ENG course" would mean guessing at a rule
  the source doesn't spell out (see lesson 3 in the curriculum-data feedback
  notes: don't invent what the source doesn't state). **Flagged for
  maintainer review.**
...
- **ENG 100.5 / MAT 150.5 (five-semester map only).** Placement-based
  ...
  Economics, History, and Sociology. Not added as additional
  required/alternative rows. **Flagged for maintainer review** if
...
1. MAT 150.5 (five-semester alternate) and ENG 100.5 are not modeled as
   additional rows; see "Prerequisite review" above.
2. The Modern Language continuation slot and the World Cultures slot can
...
3. "Any 300-level English course" as an alternate to LIN 250 is not
   encoded, since no specific course list is published for it (see
   "Choices and alternatives" above).
4. LIN 250's prerequisite could not be independently verified against a
   department course-listing page; left blank rather than guessed (see
   "Prerequisite review" above).
5. `docs/programs.csv` listed CRJ_AA's catalog year as `2026`; corrected to
   `2025-2026` per the issue and both official maps (see "Program
   identity").
6. CRJ 204's title differs between the degree maps ("Crime and Justice in
