"""Build the CUNY Beyond Phase 1 implementation record PDF."""
...
SOURCE = ROOT / "docs" / "CUNY_BEYOND_PHASE_1_IMPLEMENTATION.md"
OUTPUT = ROOT / "output" / "pdf" / "CUNY_Beyond_Phase_1_Implementation.pdf"
...
class PhaseDoc(BaseDocTemplate):
    def __init__(self):
        super().__init__(str(OUTPUT), pagesize=letter, leftMargin=.68*inch, rightMargin=.68*inch, topMargin=.7*inch, bottomMargin=.65*inch,
                         title="CUNY Beyond Phase 1 Implementation", author="Academic Advisement System")
    ...
    def decorate(canvas, doc):
        canvas.saveState()
        canvas.setFillColor(colors.HexColor("#174ea6")); canvas.rect(0, letter[1]-.18*inch, letter[0], .18*inch, fill=1, stroke=0)
        canvas.setFont("Helvetica", 8); canvas.setFillColor(colors.HexColor("#617089"))
        canvas.drawString(doc.leftMargin, .28*inch, "CUNY Beyond | Phase 1 Implementation")
        canvas.drawRightString(letter[0]-doc.rightMargin, .28*inch, f"Page {doc.page}")
        canvas.restoreState()
...
def build():
    ...
    styles = {
        ...
        "list": ParagraphStyle("List", parent=base["BodyText"], fontName="Helvetica", fontSize=9.1, leading=13, textColor=colors.HexColor("#17233c")),
        "code": ParagraphStyle("Code", parent=base["Code"], fontName="Courier", fontSize=8, leading=10.8, backColor=colors.HexColor("#edf3fa"), borderColor=colors.HexColor("#cfdded"), borderWidth=.6, borderPadding=8, spaceAfter=8),
    }
    ...
    def flush_items():
        if items:
            story.append(ListFlowable(list(items), bulletType="bullet", leftIndent=18, spaceAfter=5)); items.clear()
    ...
    for line in SOURCE.read_text(encoding="utf-8").splitlines():
        ...
        if re.match(r"^[-*] ", line):
            flush_paragraph(); items.append(ListItem(Paragraph(inline(line[2:]), styles["list"]), leftIndent=8)); continue
        numbered = re.match(r"^\d+\. (.+)$", line)
        if numbered:
            flush_paragraph(); items.append(ListItem(Paragraph(inline(numbered.group(1)), styles["list"]), leftIndent=8)); continue
        paragraph.append(line.strip())
    flush_paragraph(); flush_items()
    PhaseDoc().build(story)
