"""Build the CUNY Beyond Phase 2 implementation record PDF."""
...
SOURCE = ROOT / "docs" / "CUNY_BEYOND_PHASE_2_IMPLEMENTATION.md"
OUTPUT = ROOT / "output" / "pdf" / "CUNY_Beyond_Phase_2_Implementation.pdf"
...
class PhaseDoc(BaseDocTemplate):
    def __init__(self):
        super().__init__(str(OUTPUT), pagesize=letter, leftMargin=.68*inch, rightMargin=.68*inch, topMargin=.7*inch, bottomMargin=.65*inch,
                         title="CUNY Beyond Phase 2 Implementation", author="Academic Advisement System")
    ...
    def decorate(canvas, doc):
        ...
        canvas.setFont("Helvetica", 8); canvas.setFillColor(colors.HexColor("#617089")); canvas.drawString(doc.leftMargin, .28*inch, "CUNY Beyond | Phase 2 Implementation")
...
def build():
    ...
    def flush_items():
        if items: story.append(ListFlowable(list(items), bulletType="bullet", leftIndent=18, spaceAfter=5)); items.clear()
